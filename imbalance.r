...
adasyn <- function(form, d, k = 5, beta = 1, do_parallel = T) {
  ...
  d2 <- cbind(minor = d[, outcome_colname] == minor_clname, d[, pred_colnames],
              synthetic = F)
  ...
  if (length(to_synth_ind) > 0) {
    new_xs <- adply(1:length(to_synth_ind), 1, function(j) {
      ...
      new_xs_j <- adply(1:to_synth_n[j], 1, function(h) {
        return(synth_ref + (d2[minor_ind[sample(nn_inds, 1)],
                               pred_colnames] - synth_ref) * runif(npred))
      })[, -1]
  ...
  if (length(to_synth_ind) > 0) {
    new_xs <- adply(1:length(to_synth_ind), 1, function(j) {
      ...
      new_xs_j <- adply(1:s, 1, function(h) {
        return(synth_ref + (d2[minor_ind[nn_inds_smp[h]],
                               pred_colnames] - synth_ref) * runif(s))
      })[, -1]
...
######################################################################
set.seed(20150613)
ex_init1 <- data.frame(mvrnorm(1600, c(10, 10),
                               cor2cov(matrix(c(1, 0.3, 0.3, 1), 2, 2), c(4, 4))))
ex_init2 <- data.frame(mvrnorm(400, c(15, 15),
                               cor2cov(matrix(c(1, -0.7, -0.7, 1), 2, 2), c(1.2, 1.2))))
ex_init3 <- data.frame(mvrnorm(1200, c(5, 6),
                               cor2cov(matrix(c(1, 0.1, 0.1, 1), 2, 2), c(2, 2))))
ex_init = rbind(ex_init1, ex_init2, ex_init3)
colnames(ex_init) = c("x1", "x2")

ex_minor_ref1_x <- mvrnorm(100, c(7, 12),
                           cor2cov(matrix(c(1, 0.5, 0.5, 1), 2, 2), c(1.5, 0.75)))
ex_minor_ref2_x <- mvrnorm(50, c(13, 6),
                           cor2cov(matrix(c(1, -0.2, -0.2, 1), 2, 2), c(0.5, 1)))
...
ggplot(ex, aes(x1, x2)) +
  geom_point(size = 1.2, aes(color = type, shape = type)) +
...
bad2 <- data.frame(pred = c(rep("A", 50), rep("B", nrow(ex) - 60), rep("A", 10)),
                   obs = ex$type)
...
# more plots
######################################################################
grid.arrange(ggplot(filter(ex, type == "B"), aes(x1, x2)) +
               geom_point(size = 1.1, aes(color = type), shape = 17) +
               xlim(c(0, 20)) + ylim(c(0, 20)) +
               scale_colour_manual(values = c("blue")) +
               theme(legend.position = c(0.9, 0.9)),
             ggplot(filter(ex, type == "A"), aes(x1, x2)) +
               geom_point(size = 1.1, aes(color = type, shape = type)) +
               xlim(c(0, 20)) + ylim(c(0, 20)) +
               scale_colour_manual(values = c("red")) +
               theme(legend.position = c(0.9, 0.9)), ncol = 2)
...
p1 <- ggplot(ex_with_safety, aes(x1, x2)) +
  geom_point(size = 1, aes(color = type, shape = type), alpha = 0.25) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) +
  theme(legend.position = c(0.9, 0.9)) +
  annotate("point", ex_with_safety[safe, "x1"],
           ex_with_safety[safe, "x2"], size = 1, color = "red") +
  theme(legend.position = "none")

p2 <- ggplot(ex_with_safety, aes(x1, x2)) +
  geom_point(size = 1, aes(color = type, shape = type), alpha = 0.25) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) +
  theme(legend.position = c(0.9, 0.9)) +
  annotate("point", ex_with_safety[borderline, "x1"],
           ex_with_safety[borderline, "x2"], size = 1, color = "red") +
  theme(legend.position = "none")

p3 <- ggplot(ex_with_safety, aes(x1, x2)) +
  geom_point(size = 1, aes(color = type, shape = type), alpha = 0.25) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) +
  theme(legend.position = c(0.9, 0.9)) +
  annotate("point", ex_with_safety[noise, "x1"],
           ex_with_safety[noise, "x2"], size = 1, color = "red") +
  theme(legend.position = "none")
...
# example resampling
######################################################################
p_ex_none <- ggplot(ex, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = "none")
p_ex_none_alpha <- ggplot(ex, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type), alpha = 0.25) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = "none")
...
p_ex_under <- ggplot(ex_under, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
grid.arrange(p_ex_none, p_ex_under, ncol = 2)

# under (20%)
ex_ubunder2 <- ubUnder(ex[, 1:2], ifelse(ex$type == "A", 1, 0),
                       perc = 20, method = "percPos")
str(ex_ubunder2)
ex_under2 <- cbind(ex_ubunder2$X, type = ex_ubunder2$Y)
ex_under2$type <- as.factor(ifelse(ex_under2$type == 1, "A", "B"))
p_ex_under2 <- ggplot(ex_under2, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_over <- ggplot(ex_over, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type), alpha = 0.25) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
# SMOTE
ex_ubsmote <- ubSMOTE(ex[, 1:2], as.factor(ifelse(ex$type == "A", 1, 0)),
                      perc.over = 200, perc.under = 1000)
str(ex_ubsmote)
ex_smote <- cbind(ex_ubsmote$X, type = ex_ubsmote$Y)
ex_smote$type <- as.factor(ifelse(ex_smote$type == 1, "A", "B"))
p_ex_smote <- ggplot(ex_smote, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
summary(ex_adasyn)
p_ex_adasyn <- ggplot(ex_adasyn, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_rose <- ggplot(ex_rose, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type), na.rm = T) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
summary(ex_bsmote)
p_ex_bsmote <- ggplot(ex_bsmote, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_tomek = ggplot(ex_tomek, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_smotetomek <- ggplot(ex_smotetomek, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_ncl <- ggplot(ex_ncl, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
p_ex_bsmotencl <- ggplot(ex_bsmotencl, aes(x1, x2)) +
  geom_point(size = 1.1, aes(color = type, shape = type)) +
  xlim(c(0, 20)) + ylim(c(0, 20)) +
  scale_colour_manual(values = c("red", "blue")) + theme(legend.position = c(0.9, 0.85))
...
# load mediastinal
######################################################################
med_ids <- unname(unlist(c(read.csv("data/med_ids.csv", header = F,
                                    stringsAsFactors = F))))
...
######################################################################
resamp_methods <- expand.grid(algo = "rf", do_mad = F,
                              resamp_when = c("after_vs" , "before_vs"),
                              which = c("None", "Over2", "Over4", "Over0", "Under25",
                                        "Under50", "SMOTE_2_2", "SMOTE_3_1", "SMOTE_1_3",
                                        "Tomek", "NCL3", "NCL4", "ROSE", "ADASYN",
                                        "bSMOTE", "SMOTE_2_2_Tomek", "Tomek_SMOTE_2_2",
                                        "SMOTE_2_2_NCL3", "NCL3_SMOTE_2_2"))
resamp_methods <- resamp_methods[-2,]
resamp_methods <- rbind(expand.grid(algo = "nb", do_mad = c(F, T),
                                    resamp_when = "after_vs", which = "None"),
                        resamp_methods)
resamp_methods$run <- 1:nrow(resamp_methods)
resamp_methods <- filter(resamp_methods, resamp_when == "before_vs", algo == "rf",
                         which != "None")
...
(cv_res <- ddply(resamp_methods, .(run), function(method) {
  ...
  loocv_res <- ddply(med2, .(id), function(r) {
    ...
    if (method$which != "None") {
      ...
      if (method$which == "Over2") {
        ...
        med_train_ub <- ubUnder(med_train[, train_genes], response_vec,
                                method = "percPos", perc = 25)
      } else if (method$which == "Under50") {
        med_train_ub <- ubUnder(med_train[, train_genes], response_vec,
                                method = "percPos", perc = 50)
      } else if (method$which == "SMOTE_2_2") {
        med_train_ub <- ubSMOTE(med_train[, train_genes], response_vec,
        ...
        med_train_ub <- ubTomek(med_train[, train_genes], response_vec)
        med_train_ub <- ubSMOTE(med_train_ub$X, med_train_ub$Y,
                                perc.over = 200, perc.under = 200)
      ...
      if (method$resamp_when == "before_vs") {
        train_genes <- get_snr_genes(med_train, train_genes, m)
      }
    }

    if (method$algo == "nb") {
      fold_res <- NaiveBayes(x = med_train[, train_genes],
                             grouping = med_train$type, usekernel = T)
      r_nb_predict <- predict(fold_res, r[, train_genes])$class
    } else if (method$algo == "rf") {
      fold_res <- randomForest(x = as.matrix(med_train[, train_genes]),
                               y = med_train$type, ntree = 501)
  ...
  f1 <- unname(2 * (cm$byClass["Pos Pred Value"] * cm$byClass["Sensitivity"]) /
                 (cm$byClass["Pos Pred Value"] + cm$byClass["Sensitivity"]))
  g <- sqrt(cm$byClass["Sensitivity"] * cm$byClass["Specificity"])

  data.frame(algo = method$algo, do_mad = method$do_mad, resamp_when = method$resamp_when,
             which = method$which, tp = cm$table["MLBCL", "MLBCL"],
             fn = cm$table["MLBCL", "DLBCL"], fp = cm$table["DLBCL", "MLBCL"],
             tn = cm$table["DLBCL", "DLBCL"], accuracy = cm$overall["Accuracy"],
             bal_accuracy = cm$byClass["Balanced Accuracy"], f1 = f1, g = g,
             kappa = cm$overall["Kappa"], sensitivity = cm$byClass["Sensitivity"],
             specificity = cm$byClass["Specificity"], ppv = cm$byClass["Pos Pred Value"],
